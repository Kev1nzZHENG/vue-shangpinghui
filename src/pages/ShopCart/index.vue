<template>
  <div class="cart">
        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a
              href="#none"
              class="sindelet"
              @click.prevent="deleteCartById(cart)"
              >删除</a
            >
            <br />
            <a href="#none">移到收藏</a>
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a href="#none" @click.prevent="deleteAllCheckedCart">删除选中的商品</a>
      ...
      <div class="money-box">
        ...
        <div class="sumbtn">
          <a class="sum-btn" href="#" @click.prevent="goTrade">结算</a>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    // 修改某一个产品个数
    // type:为了区分三个元素  +：add   -:minus    input框：change
    // disNum: + 变化量（1）   -变化量（-1）  input框：最终的个数（并不是变化量）
    // cart：哪一个产品
    // 节流控制
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartInfoList.forEach((item) => {
        // console.log(item);
        if (item.isChecked == 1) {
          sum = sum + item.skuNum * item.cartPrice;
        }
      });
      return sum;
    },
